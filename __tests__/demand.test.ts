...
describe("canViewDemand", () => {
  ...
  it("APROVADOR vê demanda em AGUARDANDO_HOMOLOGACAO", () => {
    expect(canViewDemand(user("APROVADOR"), demand("AGUARDANDO_HOMOLOGACAO"))).toBe(true);
  });

  it("APROVADOR não vê demanda em ABERTA", () => {
    expect(canViewDemand(user("APROVADOR"), demand("ABERTA"))).toBe(false);
  });

  it("FINANCEIRO vê demanda HOMOLOGADA_PRODUCAO", () => {
    expect(canViewDemand(user("FINANCEIRO"), demand("HOMOLOGADA_PRODUCAO"))).toBe(true);
  });

  it("FINANCEIRO não vê demanda em EM_DESENVOLVIMENTO", () => {
    expect(canViewDemand(user("FINANCEIRO"), demand("EM_DESENVOLVIMENTO"))).toBe(false);
  });
});
...
describe("canEditDemand", () => {
  ...
  it("GESTOR não pode editar demanda de outro", () => {
    expect(canEditDemand(user("GESTOR", "g1"), demand("ABERTA", { creatorId: "g2" }))).toBe(false);
  });
  ...
  it("DEV não pode editar demanda de outro dev", () => {
    expect(canEditDemand(user("DEV", "dev1"), demand("EM_DESENVOLVIMENTO", { assigneeId: "dev2" }))).toBe(false);
  });

  it("APROVADOR não pode editar", () => {
    expect(canEditDemand(user("APROVADOR"), demand())).toBe(false);
  });

  it("FINANCEIRO não pode editar", () => {
