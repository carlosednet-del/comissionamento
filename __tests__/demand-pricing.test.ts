...
// ── Tipos de demanda precificáveis ─────────────────────────────────────────

describe("demandTypeGeneratesValue", () => {
  it("CORRECAO não gera valor", () => {
    expect(demandTypeGeneratesValue("CORRECAO")).toBe(false);
  });

  it("NOVA_SOLUCAO gera valor", () => {
    expect(demandTypeGeneratesValue("NOVA_SOLUCAO")).toBe(true);
  });

  it("EVOLUCAO_PRODUCAO gera valor", () => {
    expect(demandTypeGeneratesValue("EVOLUCAO_PRODUCAO")).toBe(true);
  });

  it("AUTOMACAO gera valor", () => {
    expect(demandTypeGeneratesValue("AUTOMACAO")).toBe(true);
  });

  it("tipo indefinido gera valor (default)", () => {
    expect(demandTypeGeneratesValue(null)).toBe(true);
    expect(demandTypeGeneratesValue(undefined)).toBe(true);
  });
});
...
describe("labels", () => {
  it("WORKER_PROFILE_LABELS cobre os 4 perfis", () => {
...
describe("isTechnicalRole", () => {
  it("retorna true para DEV",       () => expect(isTechnicalRole("DEV")).toBe(true));
  it("retorna true para SUPORTE",   () => expect(isTechnicalRole("SUPORTE")).toBe(true));
  it("retorna true para ARQUITETO", () => expect(isTechnicalRole("ARQUITETO")).toBe(true));
  it("retorna false para ADMIN",    () => expect(isTechnicalRole("ADMIN")).toBe(false));
  it("retorna false para GESTOR",   () => expect(isTechnicalRole("GESTOR")).toBe(false));
  it("retorna false para APROVADOR",() => expect(isTechnicalRole("APROVADOR")).toBe(false));
  it("retorna false para FINANCEIRO",() => expect(isTechnicalRole("FINANCEIRO")).toBe(false));
});
