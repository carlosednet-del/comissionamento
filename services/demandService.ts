...
async function buildPricingSnapshot(
  ...
  // Tipos não precificáveis (ex.: correção) não geram valor,
  // independentemente do papel ou perfil do responsável.
...
export const demandService = {
  ...
  async createDemand(input: CreateDemandInput, actor: UserForPermission) {
    ...
    // Usuários comuns só criam demandas para si: não podem atribuir a outro.
    // Apenas ADMIN e GESTOR podem definir um responsável diferente de si.
    if (!canAssignToOthers(actor) && demandData.assigneeId && demandData.assigneeId !== actor.id) {
  ...
  async updateDemand(id: string, input: UpdateDemandInput, actor: UserForPermission) {
    ...
    if (!isTechnicalRole(actor.role) && (data.assigneeId || data.estimatedHours || data.complexity || data.roi || data.demandType)) {
      pricingUpdate = await buildPricingSnapshot(
        ...
        data.demandType      ?? existing.demandType,
      );
    }
